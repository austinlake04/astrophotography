...
def master_calibration(file_tree: dict[list[str]], quiet: bool = False) -> tuple[Optional[np.ndarray]]:
    ...
    try:
        ...
        if len(file_tree["Bias"]):
            for (i, bias_file) in enumerate(tqdm(file_tree["Bias"], desc="Loading bias frames", disable=quiet)):
                with rawpy.imread(bias_file) as raw:
                    if master_bias is None: master_bias = np.empty((len(file_tree["Bias"]), *raw.raw_image.shape))
    ...
    try:
        ...
        if len(file_tree["Dark"]):
            for (i, dark_file) in enumerate(tqdm(file_tree["Dark"], desc="Loading dark frames", disable=quiet)):
                with rawpy.imread(dark_file) as raw:
                    if master_dark is None: master_dark = np.empty((len(file_tree["Dark"]), *raw.raw_image.shape))
    ...
    try:
        ...
        if len(file_tree["Dark Flat"]):
            for (i, flat_file) in enumerate(tqdm(file_tree["Dark Flat"], desc="Loading dark flat frames", disable=quiet)):
                with rawpy.imread(flat_file) as raw:
                    if master_dark_flat is None: master_dark_flat = np.empty((len(file_tree["Dark Flat"]), *raw.raw_image.shape))
                    master_dark_flat[i] = raw.raw_image
        if master_dark_flat is not None:
            master_dark_flat = np.median(master_dark_flat, axis=0)
            if master_bias: master_dark_flat -= master_bias
    except rawpy.LibRawError:
    ...
    try:
        ...
        if len(file_tree["Flat"]):
            for (i, flat_file) in enumerate(tqdm(file_tree["Flat"], desc="Loading flat frames", disable=quiet)):
                with rawpy.imread(flat_file) as raw:
                    if master_flat is None: master_flat = np.empty((len(file_tree["Flat"]), *raw.raw_image.shape))
                    master_flat[i] = raw.raw_image
        if master_flat is not None:
            master_flat = np.median(master_flat, axis=0)
            if master_bias: master_flat -= master_bias
            if master_dark_flat: master_flat -= master_dark_flat
    except rawpy.LibRawError:
...
def load(raw_file: str,
    ...
    descriptors : Optional[tuple[cv2.Descriptor]]
        Descriptors from features of interest for calibrated image
    """
    assert feature_detector in ["ORB", "SIFT", "AKAZE"], "Invalid feature detector type."
...
def calibrate(image: np.ndarray,
              master_dark: Optional[np.ndarray] = None,
...
def display(image: np.ndarray, filename: Optional[str] = None, save: bool = False) -> None:
    """Displays an image
    ...
    None
    """
    # assert save and filename is not None, "Save feature is enabled, but no filename was provided."
...
def preview(raw_file):
    with rawpy.imread(raw_file) as raw:
        return raw.raw_image

def register(image: np.ndarray,
             features,
             base_features,
             feature_detector: str = "ORB",
             match_threshhold: float = 0.8) -> np.ndarray[int]:
    ...
    matcher = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True) if feature_detector != "SIFT" else cv2.BFMatcher(cv2.NORM_L2, crossCheck=True)
...
def stack(file_tree,
    ...
    file_tree : dict[list[str]]
        ...
        {'Light': [...], 'Bias': [...], 'Dark': [...], 'Dark Flat': [...], 'Flat': [...]}
    ...
    try:
        ...
        for (i, raw_file) in enumerate(tqdm(file_tree["Light"], desc="Loading images", disable=quiet)):
            image, keypoints, descriptors = load(raw_file=raw_file,
            ...
            if image is not None: 
                if image_stack is None:
                    ...
                    image_stack[0], base_keypoints, base_descriptors = image, keypoints, descriptors
                else:
                    registered_image = register(image,
                                                features=(keypoints, descriptors),
                                                base_features=(base_keypoints, base_descriptors),
                                                feature_detector=feature_detector)
                    image_stack[i] = registered_image
        if image_stack is not None:
            stacked_image = np.sum(image_stack, axis=0)#sigmaclip(np.sum(image_stack, axis=0), 5, 5)[0]
