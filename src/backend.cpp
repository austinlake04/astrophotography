...
using cv::KeyPoint;
using std::array;
using std::cerr; using std::chrono::time_point;
...
vector<string> select_files(const string pattern) {
    glob_t globbuf;
...
public:
    ...
    QImage Backend::create_qimage(const image_t& image, const bool thumbnail) {
    ...
    */
    ...
    void Backend::load_frames(vector<string>& files, image_type_t type) {
    ...
    void Backend::create_rgb_frames() {
        switch (color_mode) {
            case color_mode_t::colored:
    ...
    void Backend::create_master_frames() {
        auto average = [](
    ...
    void Backend::set_preview(image_t& image) { preview_image = std::make_shared<image_t>(image); }
...
void display_frame(const image_t& image) {
